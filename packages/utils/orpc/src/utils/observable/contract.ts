...
// toAsyncIteratorFromObservable is defined as an exported function below.
// The local version above was replaced with the public export.

/**
 * Convert an RxJS Observable to an AsyncIterable.
 * This is the inverse of reconstructObservableFromEventIterator.
 * Each emitted value is yielded as the next iteration result.
 */
export function toAsyncIteratorFromObservable<TValue>(
  ...
  const iterator: AsyncIterator<TValue> & AsyncIterable<TValue> = {
    async next() {
      ...
      if (pendingError !== null) {
        ...
        throw pendingError instanceof Error ? pendingError : new Error("Observable to AsyncIterable error");
      }
    ...
    throw(error: unknown) {
      subscription.unsubscribe();
      return Promise.reject(error instanceof Error ? error : new Error("Observable to AsyncIterable error"));
    },
