...
export function DataTable<TData extends ExportableData, TValue>({
  ...
  const getSelectedItems = useCallback(async () => {
    ...
    const isNumericIds = !!firstDataItem && typeof firstDataItem[idField] === 'number';
            ...
            {isLoading ? (
              ...
              table.getRowModel().rows.map((row, rowIndex) => {
                ...
                if (subRowsConfig?.enabled && subRowsConfig.mode === 'custom-columns' && isSubRow && subRowColumns) {
                  ...
                  const previousRow = prevRowIndex >= 0 ? flatRows[prevRowIndex] : undefined;
                  const isFirstSubRow = previousRow?.id === parentRow.id;
                      ...
                      <React.Fragment key={`subrow-group-${row.id}`}>
                        ...
                        >
                          {subRowColumns.map((column, colIndex) => {
                            ...
                            const sortDirection = isSorted ? currentSort?.direction ?? null : null;
                ...
                return (
                  <TableRow
                    ...
                    onClick={(event) => {
                      ...
                      if (subRowsConfig?.enabled && row.getCanExpand()) {
                        row.toggleExpanded();
                      }
                    ...
                    style={{
                      cursor: (onRowClick || (subRowsConfig?.enabled && row.getCanExpand())) ? 'pointer' : undefined
                    }}
                  >
